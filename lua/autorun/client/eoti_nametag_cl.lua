    ...
    function EOTI_NameTag.drawNames(target,dist)
        if #EOTI_NameTag.NearbyPlayers > 0 then
            ...
            for _, ply in pairs(EOTI_NameTag.NearbyPlayers) do
                if ply:IsValid() and LocalPlayer():IsLineOfSightClear(ply) and !(ply:GetNWBool( "StealthCamo", nil )) and (target != ply or dist > EOTI_NameTag.Distance) then
		...
		cam.IgnoreZ(true)
            ...
            if valid then
                ...
                if (isply or isnpc or isveh or isprop) then
                    ...
                    if DarkRP and isply and target:getDarkRPVar('wanted') or dist < EOTI_NameTag.Distance and !(target:GetNWBool( "StealthCamo", nil )) then
